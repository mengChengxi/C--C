...
struct Info {
    ...
    Info() {}
    // 单点初始化的构造
    Info(long long val) : len(1) ,arr(1,val){}
    ...
    friend Info operator+( const Info& a, const Info& b) {
        if(k!=INF){
            Info c;
            auto it = upper_bound(a.arr.begin(), a.arr.end(), k);//first element > i
            tk+=a.arr.end()-it;
            it = upper_bound(b.arr.begin(), b.arr.end(), k);//first element > i
            tk+=b.arr.end()-it;
            return c;
        }
        Info c;
        std::merge(a.arr.begin(), a.arr.end(), 
           b.arr.begin(), b.arr.end(), 
           std::back_inserter(c.arr));
