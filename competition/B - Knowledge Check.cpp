...
struct Info {
    ...
    friend Info operator+(const Info& a, const Info& b) {
        ...
        int ap=0;
        int bp=0;
        while(ap!=a.arr.size()&&bp!=b.arr.size()){
            if(ap==a.arr.size()){
                temp.push_back(b.arr[bp]);
                continue;
            }
            if(bp==b.arr.size()){
                temp.push_back(a.arr[ap]);
                continue;
            }

            if(a.arr[ap]<b.arr[bp]){
                temp.push_back(a.arr[ap]);
            }else{
                temp.push_back(b.arr[bp]);
            }
...
void solve() {
    ...
    while(q--){
        ...
        if(o==0){
            int p,x;
            cin>>p>>x;
            st.modify(p, p, Tag{x}); // 数组变为 {1, 7, 5, 6, 2}
        }else{
            int l,r;
            cin>>l>>r;
            
            Info res2 = st.query(l, r-1);
            cout << res2.sum << endl;       // 输出 18
        }
    }
